...
class StaffFeedbackUpdateSerializer(serializers.ModelSerializer):
    updated_by_username = serializers.CharField(source="updated_by.username", read_only=True)
    assigned_to_username = serializers.CharField(source="assigned_to.username", read_only=True)

    class Meta:
        model = StaffFeedbackUpdate
        fields = [
            "id",
            "previous_status",
            "new_status",
...
class StaffFeedbackNotificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = StaffNotification
...
class StaffFeedbackSerializer(serializers.ModelSerializer):
    staff = serializers.CharField(source="staff_name")
    ...
    assigned_to_username = serializers.CharField(source="assigned_to.username", read_only=True)
    updated_by_username = serializers.CharField(source="updated_by.username", read_only=True)
    updates = StaffFeedbackUpdateSerializer(many=True, read_only=True)
    notification_history = StaffFeedbackNotificationSerializer(source="notifications", many=True, read_only=True)
    ...
    def validate(self, attrs):
        ...
        if self.instance is None:
            forbidden_fields = []
            if attrs.get("status") and attrs["status"] != StaffFeedback.Status.PENDING:
            ...
            attrs["status"] = StaffFeedback.Status.PENDING
        elif not is_manager:
